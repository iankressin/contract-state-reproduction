...
- Abort LATENCY on the PostgresSink path is up to ~300ms: the SDK runs onData inside `doWithRetry(db.transaction(...), {retries:3, delayMs:100})`, which does NOT special-case AbortError, so a throwIfAborted() is retried 3×(~100ms) — re-running the aborted batch tx each time — before it escapes pipeTo; clean resolution (no throw, partial results committed) still holds, cancellation is just delayed.
