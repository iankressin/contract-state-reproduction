...
// The committed DAI job (pinned inline shapes — solc 0.5.12 predates storageLayout).
...
const plans = await resolvePlans(undefined, trackedVariables)
const planOf = (v: string) => plans.find((p) => p.variable === v)
const pg = new Client({ connectionString: process.env.DB_URL })
await pg.connect()
const client = createPublicClient({ transport: http(process.env.RPC_URL) })

const { rows: m } = await pg.query<{ n: number }>(`SELECT MAX("blockNumber")::int AS n FROM state_value WHERE contract=$1`, [address])
const N = m[0]?.n
if (!N) throw new Error('state_value is empty — run the indexer first (pnpm example)')
...
const check = async (label: string, plan: Plan, keys: string[], db: bigint, call: Promise<bigint>) => {
  total++
  const [oracle, accessor] = await Promise.all([chainValueAt(client, address, plan, N, keys), call])
