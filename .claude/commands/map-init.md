...
- **TypeScript/JS** → `madge`. Install with `npm i -D madge`.
- **Python** → `pydeps`. Install with `pip install pydeps`.
- **Other stacks** → see `README.md` for the tool to swap in (Go → `go-callvis`, etc.)
  and wire it into `.claude/hooks/update-structure.sh`.

The raw graph is rendered to a Mermaid `graph.mmd` by `graph_to_mermaid.py` (needs
`python3`, no Graphviz). Then run `bash .claude/hooks/update-structure.sh` and confirm
`docs/context/structure/graph.json` and `graph.mmd` were produced.
...
Then **group the lines under `##` headers naming the kind of constraint**, deriving those
kinds from the invariants you actually found (e.g. "Data model", "Decode limits", "Build &
deps") rather than a fixed list. A header is a kind — a noun phrase — never a subsystem or
module name; keep it one level deep, no sub-sections.
...
- Every inferred invariant phrased as a **yes/no question**, **grouped by kind** so each
  cluster is reviewed together ("Confirm: ingest assumes blocks arrive in order? (y/n)").
